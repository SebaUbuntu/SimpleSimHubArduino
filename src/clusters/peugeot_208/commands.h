...
static void sendIgnitionAndLighting(
	...
	messageDebouncer.sendMessageWithDebounce(mcp2515, frame);
}
...
static void sendIgnitionAndCoolantTempAndOdometerAndAmbientTempAndReverseAndTurnSignals(
	...
	frame.data[5] = 0x6E; // Do not change
	frame.data[6] = ambientTemperatureCelsius; // Ambient temperature
	frame.data[7] = 0x20
		| (gear == Gear::GEAR_R ? 0x80 : 0x00) // Bit 7: Reverse gear
		| (rightIndicator ? 0x02 : 0x00) // Bit 1: Turn right
		| (leftIndicator ? 0x01 : 0x00); // Bit 0: Turn left
...
static void sendTripMeter(
	...
	frame.data[5] = (tripMeterMeters >> 16) & 0xFF;
	frame.data[6] = (tripMeterMeters >> 8) & 0xFF;
	frame.data[7] = tripMeterMeters & 0xFF;
...
static void sendDashboardLights(
	...
	frame.can_dlc = 8;
	frame.data[0] = 0x00
		| (sidelights ? 0x80 : 0x00) // Bit 7: Sidelights
		| (lowBeams ? 0x40 : 0x00) // Bit 6: Low beams
		| (highBeams ? 0x20 : 0x00) // Bit 5: High beams
		| (frontFogLights ? 0x10 : 0x00) // Bit 4: Front fog lights
		| (rearFogLights ? 0x08 : 0x00) // Bit 3: Rear fog lights
		| (rightIndicator ? 0x04 : 0x00) // Bit 2: Right indicator
		| (leftIndicator ? 0x02 : 0x00); // Bit 1: Left indicator
	...
	frame.data[3] = 0x00
		...
		| (serviceLight ? 0x80 : 0x00); // Bit 7: Service light on
	frame.data[4] = 0x00
...
static void sendWarningLights(
	...
	EngineOilLevel engineOilLevel,
	bool highEngineCoolantTemperature,
	...
	bool powerSteeringWarningLight,
	bool airbagWarningLight,
	bool lowBeamsWarningLight,
	bool waterInFuelFilterWarningLight
) {
	static MessageDebouncer messageDebouncer(200);
	struct can_frame frame;
	...
	frame.can_dlc = 8;
	frame.data[0] = 0x00
		| (highEngineCoolantTemperature ? 0x80 : 0x00) // Bit 7: High engine coolant temperature
	...
	frame.data[4] = 0x00
		| (waterInFuelFilterWarningLight ? 0x80 : 0x00) // Bit 7: Water in fuel filter warning light
		| (lowBeamsWarningLight ? 0x40 : 0x00) // Bit 6: Low beams blink
		| (airbagWarningLight ? 0x20 : 0x00) // Bit 5: Airbag warning light
		| (powerSteeringWarningLight ? 0x04 : 0x00) // Bit 2: Power steering warning light
